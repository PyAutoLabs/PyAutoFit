...
class _TaggedSamplesPDF(af.SamplesPDF):
    """Proves test-mode recovery retains the sampler's concrete sample type."""
...
class TestReducedModeRejectedFinalSample:
    ...
    def test__test_mode_1__factor_graph_children_and_sample_type_are_preserved(
        self, monkeypatch
    ):
        monkeypatch.setenv("PYAUTO_TEST_MODE", "1")
        ...
        rejected_samples = _TaggedSamplesPDF(
            model=model,
            sample_list=af.Sample.from_lists(
                model=model,
                parameter_lists=[[0.1]],
                log_likelihood_list=[-1.0e99],
                log_prior_list=[0.0],
                weight_list=[1.0],
            ),
            samples_info={"log_evidence": -1.0e99, "sampler_marker": "retained"},
        )
        ...
        assert result.samples.samples_info["sampler_marker"] == "retained"
        assert len(result) == 2
