...
def test__chunked_update_strategy_factory():
    ...
    whether ``chunk_size`` is set. This lets ``build_chunked_nss_algorithm``
    drop it into the kernel composition without further branching.
    """
...
def test__init_raises_when_nss_unavailable(monkeypatch):
    ...
    with pytest.raises(ImportError, match="af.NSS requires blackjax >= 1.6"):
        af.NSS()
