...
def test__log_posteriors_are_aligned_with_the_thinned_chain(monkeypatch):
    """
    The parameters and the log posteriors must come out of ``emcee`` under the
    *same* ``discard`` / ``thin``, so that sample ``i``'s log likelihood is the
    ...
    probabilities were a contiguous tail slice of the raw, unthinned array (plus
    a one-element offset), so the two were not in correspondence and the reported
    maximum log likelihood belonged to a different point in parameter space
    (PyAutoFit#1628).
